...
plot(legameMediana,hang=-1, xlab="Metodo gerarchico agglomerativo",
     sub="del legame medio")
axis(side=4,at=round(c(0,legameMediana$height),2),las=1)
rect.hclust(legameMediana,k=3,border="red")
...
qualità.taglio<-function(gruppi){
  taglio<-cutree(legameMediana,k=gruppi,h=NULL)
  num<-table(taglio )
...
plot(c(0,legameMediana$height),type="b",col="red",
     main="Screeplot metodo della mediana",
     xlab = "Distanza di aggregazione",
     ylab="Numero di cluster")

cutree(legameMediana, k=3, h=NULL)