...
plot(legameCentroid,hang=-1, xlab="Metodo gerarchico agglomerativo",
     sub="del centroide")
axis(side=4,at=round(c(0,legameCentroid$height),2),las=1)
rect.hclust(legameCentroid,k=3,border="red")
...
qualità.taglio<-function(gruppi){
  taglio<-cutree(legameCentroid,k=gruppi,h=NULL)
  num<-table(taglio )
...
plot(c(0,legameCentroid$height),type="b",col="red",
     main="Screeplot metodo centroide",
     xlab = "Distanza di aggregazione",
     ylab="Numero di cluster")


cutree(legameCentroid, k=3, h=NULL)